...
So we now known *roughly* how the ColBERT architecture works; Query text  is encoded into a fixed length bag of token embeddings and document text is encoded into a bag of token embeddings. But the missing piece is how do we compute the relevancy score of a query, document pair using this representation? 
...
The cosine similarity with unit length vectors can be performed by the the inner dot product, 
